...
class AppointmentServiceImpl @Inject()(
  ...
  override def search(query: AppointmentSearchQuery, limit: Int)(implicit t: TimingContext): Future[ServiceResult[Seq[Appointment]]] =
    daoRunner.run(dao.searchQuery(query).take(limit).result)
      .map(_.filter(a => query.location.forall(q => a.location.exists(l => l.name.toLowerCase.contains(q.name.toLowerCase)))))
      .map(r => Right(r.map(_.asAppointment)))
...
object AppointmentService {
  def groupTuples(withClients: Seq[(Appointment, AppointmentClient)], withCase: Seq[(Appointment, Option[Case])], withNotes: Seq[(Appointment, Option[StoredAppointmentNote])]): Seq[AppointmentRender] = {
    val appointmentsAndClients = OneToMany.join(withClients)(Ordering.by[AppointmentClient, String](_.universityID.string))
