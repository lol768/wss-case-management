...
package object services {
  ...
  def updateDifferencesDBIO[A, B](items: Set[B], query: Query[Table[A], A, Seq], map: A => B, comap: B => A, insert: Set[A] => DBIO[Seq[A]], delete: Set[A] => DBIO[Seq[Done]])(implicit ec: ExecutionContext): DBIO[UpdateDifferencesResult[A]] = {
