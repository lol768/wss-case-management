...
trait CaseService {
  ...
  def getNote(id: UUID)(implicit t: TimingContext): Future[ServiceResult[CaseNote]]
...
class CaseServiceImpl @Inject() (
  ...
  def getNote(id: UUID)(implicit ac: AuditLogContext): Future[ServiceResult[CaseNote]] =
    daoRunner.run(dao.findNote(id)).map { n => Right(n.asCaseNote) }
    ...
    ownerService.getCaseOwnerHistory(id).flatMap(result => result.fold(
      ...
      rawOwnerHistory => {
        daoRunner.run(for {
          ...
          CaseHistory.apply(caseHistory, rawTagHistory, rawOwnerHistory, rawClientHistory, userLookupService, clientService)
        }
      }
    ))
  }
