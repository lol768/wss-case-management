...
import domain.{CaseLinkType, CaseTag, Fixtures}
...
class CaseServiceTest extends AbstractDaoTest {
  ...
  "CaseServiceTest" should {
    ...
    "get and set links" in withData(new CaseFixture()) { c1 =>
      ...
      service.addLink(CaseLinkType.Related, c1.id.get, c2.id.get).serviceValue
      service.addLink(CaseLinkType.Merge, c1.id.get, c3.id.get).serviceValue
      service.addLink(CaseLinkType.Merge, c2.id.get, c3.id.get).serviceValue
